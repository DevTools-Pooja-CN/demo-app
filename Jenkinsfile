pipeline {
    ...
    environment {
        ...
        GITHUB_REPO        = "DevTools-Pooja-CN/demo-app"
        ISSUE_ASSIGNEE     = "Pooja-DevTools"
    }
        ...
        stage('Upload ZAP Report to JFrog Artifactory') {
            steps {
                withCredentials([
                    ...
                    sh """
                       jf c add my-jfrog-server --url http://130.131.164.192:8082 --user $JFROG_USER --password $JFROG_PASS --interactive=false --overwrite=true
                        jf rt upload "zap_report.*" "art-docker-local/zap-reports/${BUILD_NUMBER}/" --server-id=dev-artifactory
                    """
                }
            }
        }
